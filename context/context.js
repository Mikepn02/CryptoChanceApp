...
export const AppProvider = ({ children }) => {
  ...
  const [lotteryAddress, setLotteryAdress] = useState()
  ...
  const updateState = async () => {
    ...
    try {
      ...
      setLotteryAdress(lotteryAddress)
      ...
      // check if any of user tickets win:
  ...
  const getHistory =  async() => {
    if(!lotteryId) return;
    const history =[];
    for(const i in new Array(lottery).fill(null)){
  ...
  const pickWinner = async () => {
    try {
      ...
      await confirmTx(txHash, connection)
      updateState()
      toast.success("picked the winner")
    } catch (err) {
      toast.error(err.message)
    }
  }
